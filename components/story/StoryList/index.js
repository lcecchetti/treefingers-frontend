...
export const QUERY_STORIES = gql`
  query stories($filter: FilterStoryInput, $sort: SortStoryInput, $query: String, $first: Int, $after: String) {
    stories(filter: $filter, sort: $sort, query: $query, first: $first, after: $after) {
      edges {
        cursor
        node {
          id
...
const StoryList = ({ className, filter, sort, first = 10, query, setTotalCount }) => {
  const { currentUser } = useCurrentUser();

  const { data, loading, error, refetch, fetchMore } = useQuery(QUERY_STORIES, {
    variables: { filter, first, sort, query },
  });
