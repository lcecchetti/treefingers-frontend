...
import { FormField, Button, Text, Spinner } from 'components/ui';
...
const StoryNew = ({ parent, forest, className }) => {
  ...
  const { data: forestsData, loading: loadingForests } = useQuery(QUERY_CHOOSE_FOREST, {
    variables: {
      filter: {
        _id: { eq: forest },
      },
  ...
  const prepareForestOptions = (data) => {
    return data?.forests.edges.map(({ node }) =>({
      value: node._id,
  ...
  return (
    <div className={className}>
      <AuthRequired>
        {!parent && forest && !forestsData &&
          <Spinner loading={loadingForests} />
        }
        {(!!parent || (!parent && forest && forestsData)) &&
          <Formik
            initialValues={{
              title: '',
              content: '',
              ...
              forests: prepareForestOptions(forestsData),
              forestsLoading: false,
            }}
